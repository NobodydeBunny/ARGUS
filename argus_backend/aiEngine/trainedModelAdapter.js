...
const getPredictScriptPath = () => {
  return path.join(getBackendRoot(), "ml_training", "predict_ui_issue.py");
};
...
const inferModuleName = (candidate) => {
  ...
  if (text.includes("color") || text.includes("contrast") || text.includes("visibility")) {
    return "color";
  }
...
const buildModelFeatures = (candidate) => {
  ...
  if (candidate.candidateType === "weak_error_visibility") {
    ...
    base.errorVisibilityScore = Math.max(
      Number(base.errorVisibilityScore || 0),
      Number(candidate.evidenceScore || 0.6)
    );
  }
...
const fallbackPrediction = (candidate) => {
  ...
  return {
    ...
    confidenceScore: Number(
      Math.max(
        0.45,
        Math.min(0.85, Number(candidate.evidenceScore || 0.65))
      ).toFixed(3)
    ),
...
const predictBatchWithTrainedModel = (candidates) => {
  ...
  try {
    if (!canUsePythonModel()) {
      return candidates.map(fallbackPrediction);
    }

    const scriptPath = getPredictScriptPath();

    const features = candidates.map(buildModelFeatures);
    ...
    return candidates.map((candidate, index) => {
      return normalizePrediction(predictions[index], candidate);
    });
  } catch (error) {
    return candidates.map(fallbackPrediction);
  }
