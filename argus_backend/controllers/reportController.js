...
const exportReportById = async (req, res) => {
  try {
    // Get report with related session and analysis
    const report = await Report.findById(req.params.id)
      .populate("sessionId")
      .populate("analysisId");
    ...
    const fileName =
      `argus-session-report-${report.sessionId._id}.txt`;

    const issueText = sessionIssues.map((issue, index) => {

      // Find suggestions linked to issue
      const relatedSuggestions = sessionSuggestions.filter(
        suggestion =>
          suggestion.issueId.toString() === issue._id.toString()
      );

      // Format suggestion details
      const suggestionText = relatedSuggestions.map(
