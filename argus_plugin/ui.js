...
const backToDashboardFromResults =
  document.getElementById("backToDashboardFromResults");
const backToDashboardFromReport =
  document.getElementById("backToDashboardFromReport");
const refreshResultsBtn =
  document.getElementById("refreshResultsBtn");
const openReportFromResultsBtn =
  document.getElementById("openReportFromResultsBtn");
...
const minorCount =
  document.getElementById("minorCount");
...
const reportTotalIssues =
  document.getElementById("reportTotalIssues");
const reportCritical =
  document.getElementById("reportCritical");
const reportModerate =
  document.getElementById("reportModerate");
const reportMinor =
  document.getElementById("reportMinor");
const reportTitle =
...
const reportTopIssues =
  document.getElementById("reportTopIssues");
...
function setStatus(message, isError = false) {
  ...
  reportStatusOutput.classList.toggle(
    "error",
    isError
  );
}
...
function requestPluginClose() {
  const confirmed = confirm(
    "Are you sure you want to stop Argus UI Analyzer?"
  );
...
/* =========================================================
   FRAME-STRUCTURED METADATA
   ========================================================= */

/*
 * This function only restructures the metadata that is
 * displayed in the existing Extracted JSON Data area.
 *
 * It does NOT modify the original data sent to the backend.
 * It does NOT modify the UI.
 */
function structureMetadataByFrame(data) {
  if (!data) {
    return {};
  }
  ...
  const structuredFrames = frames.map(
    (frame, index) => {
  ...
  if (!issues || issues.length === 0) {
    dashboardIssues.innerHTML = `
      <div class="issue-card">
        ...
        <p>
          The selected design does not show detectable
          issues based on the current analysis.
        </p>
      </div>
    `;
      ...
      issue => `
        <div class="issue-card ${getSeverityClass(
          ...
          <p>
            ${escapeHtml(
              issue.message,
              "Pattern issue detected in the selected design."
            )}
          </p>
  ...
  if (!issues || issues.length === 0) {
    resultsList.innerHTML = `
      <div class="issue-card">
        ...
        <p class="description">
          No design problems were detected in the
          latest analysis.
        </p>
      </div>
    `;
      ...
      issue => `
        <div class="issue-card ${getSeverityClass(
          ...
          <p class="description">
            ${escapeHtml(
              issue.message,
              "The AI model detected a possible UI pattern issue."
            )}
          </p>
          ...
          <div class="suggestion">
            ...
            <p>
              ${escapeHtml(
                issue.explanation ||
                "The suggestion was generated from the detected issue type and Figma metadata evidence."
              )}
            </p>
...
function updateReportSummaryFromAnalysis(
  analysis
) {
  const issues =
  ...
  reportTotalIssues.textContent =
    issues.length;

  reportCritical.textContent =
    counts.high;

  reportModerate.textContent =
    counts.medium;

  reportMinor.textContent =
    counts.low;
  ...
  if (analysis) {
    ...
    reportTitle.textContent =
      "Argus UI Design Analysis Report";
  }
...
function renderAnalysis(analysis) {
  ...
  updateReportSummaryFromAnalysis(
    analysis
  );
}
...
closePluginBtn.onclick =
  requestPluginClose;


analyzeBtn.onclick = () => {
  setStatus(
    "Analyzing selected Figma layers..."
  );
...
pauseBtn.onclick = () => {
  if (!isScanning) {
    setStatus(
      "Near real-time scanning is already paused."
    );
  ...
  setStatus(
    "Near real-time scanning paused."
  );
};
...
exportReportBtn.onclick = () => {
  ...
  const confirmed = confirm(
    "Do you want to export this report now?"
  );
  ...
  cancelExportBtn.classList.remove(
    "hide"
  );
};
...
cancelExportBtn.onclick = () => {
  if (!currentReportId) {
    setStatus(
      "No report is available to cancel.",
      true
    );
...
onmessage = event => {
  ...
  if (msg.type === "design-data") {
    ...
    /*
     * Only restructure how the JSON is displayed.
     *
     * No additional UI elements or headings are
     * created.
     */
    const structuredMetadata =
      structureMetadataByFrame(
        msg.data
      );
  ...
  if (
    ...
    reportProjectName.textContent =
      latestAnalysis
        ? latestAnalysis.designName
        : "Current Figma Selection";

    reportDate.textContent =
      new Date(
        msg.data.generatedAt ||
        Date.now()
      ).toLocaleString();

    reportStatusOutput.textContent =
      "Report generated. You can export it now.";

    reportStatusOutput.classList.remove(
      "error"
    );
